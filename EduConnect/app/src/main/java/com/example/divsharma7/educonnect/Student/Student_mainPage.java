...
public class Student_mainPage extends AppCompatActivity
    ...
    FirebaseRecyclerAdapter mAdapter; String[] maillist=new String[100];
    FirebaseUser user;
    FirebaseAuth auth; int i=0;   static String this_email;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
     ...
     /*   DatabaseReference mDatabase=FirebaseDatabase.getInstance().getReference("newsfeed");
        String uid;
        uid=auth.getCurrentUser().toString();
        // Creating new user node, which returns the unique key value
        // new user node would be /users/$userid/
        String userId = mDatabase.push().getKey();
        Feeder f=new Feeder("Divyanshu","This is my message ", uid);

        // creating user object


        // pushing user to 'users' node using the userId
        mDatabase.child(userId).setValue(f);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.student_main_page, menu);
        return true;
    }
